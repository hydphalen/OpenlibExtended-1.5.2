  ...
  try {
    ...
    if (openWithExternalApp) {
      ...
      try {
        ...
        // ignore: use_build_context_synchronously
                showSnackBar(context: context, message: AppLocalizations.of(context)!.unableToOpenEpub);
      }
    }
  } catch (e) {
              ...
              loading: () => Center(
                child: CircularProgressIndicator(
                  color: Theme.of(context).colorScheme.secondary,
                            ...
                            error: (err, stack) =>
                  Center(child: Text(AppLocalizations.of(context)!.errorLoadingBook(err.toString()))),
            ),
          ),
          if (_showTutorial)
            ReaderHelpOverlay(
